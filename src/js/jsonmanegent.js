  ...
  try {
    ...
    citas.forEach(cita => {
      if (cita.disponible) {
        ...
        botonSolicitar.textContent = 'Solicitud de agendamiento';
  ...
  try {
    ...
    solicitudes.forEach(solicitud => {
      if (!solicitud.aprobada) {
        ...
        botonRechazar.addEventListener('click', () => rechazar(solicitud.id, solicitud.citaId));

        li.appendChild(botonAprobar);
        listaSolicitudes.appendChild(li);

        
        li.appendChild(botonRechazar);
        listaSolicitudes.appendChild(li);
      }
    });
...
async function rechazar(idSolicitud, idCita) {

  alert(`Cita con ID ${idCita} rechazada. Se marcará como disponible.`);
  document.getElementById('citasPersonal').style.display = 'none';
